...
function getFallbackErrorMessage(err) {
  switch (err.__fxotype__) {
    ...
    default:
      return `The request failed because something unexpected happened (${err.message}).`;
  }
}
...
RunUtil.harvestInputs = function(grunt, runner, method, inputs, callback) {
  ...
  function doPrompts(inputSet) {
    ...
    inputSet = inputSet.map(function(input) {
      ...
      if (input.dependants) {
        input.after = function(results, done) {
          ...
          runner.run(method.slug, 'input', dataToSend, function(
            err,
            updatedInputFields
          ) {
            if (err) {
        ...
        var key = input.key;
        var manualOption =
          '__fxo_select_manual_' + key + '_' + Date.now() + '__';
      ...
      if (input.input_options) {
        ...
        if (repeatedOptions.length) {
          grunt.fail.fatal(
            "Repeated option values detected for input field '" +
              input.key +
              "': " +
              repeatedOptions +
    ...
    CommonUtil.promptFields(inputSet, fieldPromptOptions, function(
      err,
      answers
    ) {
      if (err) {
  ...
  if (!method.scripts.input) {
    ...
    runner.run(method.slug, 'input', function(err, customInputFields) {
      ...
      try {
        ...
        grunt.fail.fatal(
          'Error in return from input.js script: ' + e.toString()
        );
      }
      var combinedInputs = [];
  ...
  async.waterfall(
    [
      ...
      function(callback) {
        ...
        if (
          (method.fields.input && method.fields.input.length) ||
          method.scripts.input
        ) {
        ...
        if (inputsPredefined) {
          inputs.forEach(function(input) {
            ...
            grunt.log.writeln(
              chalk.magenta(' ' + prompt.message + ' ' + prompt.value)
            );
          });
          return callback();
        }
          ...
          function(err, customOutputs) {
            if (err) {
              ...
              try {
                ...
                grunt.fail.fatal(
                  'Error in return from output.js script: ' + e.toString()
                );
              }
  ...
  async.waterfall(
    [
      ...
      function(userScript, callback) {
        ...
        script = userScript;
        if (
          (userScript === 'run' || userScript === 'output') &&
...
RunUtil.runRecorded = function(grunt, options, cb) {
  var runFile = path.join(options.runsFolder, RunUtil.getRunFile(grunt));
...
RunUtil.runReplayed = function(grunt, options, cb) {
  var runner = options.runner,
    service = options.service,
    runFile = path.join(options.runsFolder, RunUtil.getRunFile(grunt));
...
RunUtil.runWebhook = function(grunt, options, method, callback) {
  ...
  localtunnel(options.webhookPort, localtunnelOpts, function(err, tunnel) {
    ...
    if (
      method.help &&
      method.help.webhook &&
      method.help.webhook.config &&
      method.help.webhook.config.length > 0
    ) {
    ...
    if (
      method.help &&
      method.help.webhook &&
      method.help.webhook.test &&
      method.help.webhook.test.length > 0
    ) {
    ...
    app.post('/', function(req, res) {
      ...
      options.runner.run(method.slug, 'run', { input: req.body }, function(
        err
      ) {
    ...
    var subdomain = tunnel.url.replace(/^https:\/\/(.*).localtunnel.me$/, '$1');
...
RunUtil.runTask = function(grunt, options) {
  ...
  if (service.methods.length === 0) {
    grunt.fail.fatal(
      'You have no methods to run! Create new methods with `yo flowxo:method`'
    );
  }
