...
var Service = function(options) {
  ...
  this.methodsIndexed = _.keyBy(this.methods, 'slug');
};
...
Service.prototype.toJSON = function() {
  var config = _.pick(this, ['name', 'description', 'slug', 'help']);

  if(this.hasOwnProperty('auth')) {
    config.auth = _.pick(this.auth, ['type', 'fields', 'params']);
  }

  config.scripts = scriptReducer(this.scripts);

  config.methods = this.methods.map(function(m) {
    var method = _.pick(m, ['name', 'slug', 'type', 'kind', 'fields', 'help', 'deprecated']);
    method.scripts = scriptReducer(m.scripts);
    return method;
  });
